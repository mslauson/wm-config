...
tmux send-keys -t $WORK:0 'pApi && nvim' C-m

tmux new-window -t $WORK:1 -n 'ui'
tmux send-keys -t $WORK:1 'pUi && nvim' C-m

tmux new-window -t $CONF:0 -n 'nvim'
tmux send-keys -t $CONF:0 'pConf && cd astro-config && nvim .' C-m

tmux new-window -t $CONF:1 -n 'wm'
tmux send-keys -t $CONF:1 'pConf && cd wm-config && nvim .' C-m

tmux new-window -t $CONF:2 -n 'term'
tmux send-keys -t $CONF:2 'pConf && cd term-config && nvim .' C-m
